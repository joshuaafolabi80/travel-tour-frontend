// travel-tour-frontend/src/components/MasterclassCourses.jsx - UPDATED FIXED VERSION
...
const MasterclassCourses = ({ navigateTo }) => {
  ...
  useEffect(() => {
    ...
    // 2. 🔥 FIXED: Pre-fill email if user is logged into the main app (from Gemini version)
  ...
  // 🔥 FIXED: validateAccessCode function (from Gemini version with proper validation)
  const validateAccessCode = async () => {
    ...
    try {
      // 🔥 FIXED: This route now works with the updated backend
      const response = await api.post('/courses/validate-masterclass-access', {
        accessCode: accessCode.trim().toUpperCase(), // Convert to uppercase
      ...
      if (response.data.success) {
        ...
        // Fetch courses and question sets
        await fetchCourses();
  ...
  // 🔥 FIXED: logout function (from Gemini version with proper cleanup)
  const logout = () => {
    setHasAccess(false);
    setShowAccessModal(true);
    setAccessCode('');
            ...
            <div className="col-12 col-md-6 col-lg-5">
              <div className="card shadow-lg border-warning">
                <div className="card-header bg-warning text-dark text-center py-4">
                  ...
                  <p className="lead mb-0">Premium content requiring access code</p>
                </div>
  ...
  return (
    <div className="masterclass-courses" style={{ background: '#f8f9fa', minHeight: '100vh' }}>
      <div className="container-fluid py-4">
        {/* Header - FIXED: Added user email display and logout button from Gemini version */}
                ...
                <div className="row align-items-center">
                  <div className="col-md-8">
                    ...
                    <p className="lead mb-0 opacity-75 text-dark">
                      Welcome, {localStorage.getItem('masterclassUserEmail') || userEmail}
                    </p>
                  </div>
      ...
      {/* Course Modal (EXISTING CODE - NO CHANGES) */}
